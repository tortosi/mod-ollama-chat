...
std::string GetBotPersonality(Player* bot)
{
    ...
    if (it != g_BotPersonalityList.end())
    {
        ...
        if(g_DebugEnabled)
        {
            LOG_INFO("playerbots", "[Ollama Chat] Using existing personality '{}' for bot {}", it->second, bot->GetName());
        }
        return it->second;
    }
    ...
    if (g_BotPersonalityList.find(botGuid) != g_BotPersonalityList.end())
    {
        ...
        if(g_DebugEnabled)
        {
            LOG_INFO("playerbots", "[Ollama Chat] Using database personality '{}' for bot {}", dbPersonality, bot->GetName());
        }
    ...
    if (!tableExists)
    {
        LOG_INFO("playerbots", "[Ollama Chat] Please source the required database table first");
    }
    else
    {
    ...
    if(g_DebugEnabled)
    {
        LOG_INFO("playerbots", "[Ollama Chat] Assigned new personality '{}' to bot {}", chosenPersonality, bot->GetName());
    }
...
bool SetBotPersonality(Player* bot, const std::string& personality)
{
    ...
    if(g_DebugEnabled)
    {
        LOG_INFO("playerbots", "[Ollama Chat] Set personality '{}' for bot {}", personality, bot->GetName());
    }
...
void ClearAllBotPersonalities()
{
    ...
    if(g_DebugEnabled)
    {
        LOG_INFO("playerbots", "[Ollama Chat] Cleared all bot personality assignments due to RP personalities being disabled");
    }
}
